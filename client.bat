...
if not exist %_jre_dir%\ (
	...
	set _temp_jdk_archive=%_temp_dir%\jdk-%_java_version%-windows-%_arch%-archive.zip
	
	powershell -Command^
	 "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12;"^
	 "$ProgressPreference = 'SilentlyContinue';"^
	 "Invoke-WebRequest -OutFile '%_temp_jdk_archive%' 'https://api.adoptopenjdk.net/v2/binary/releases/openjdk%_java_version%?openjdk_impl=hotspot&release=latest&type=jdk&heap_size=normal&os=windows&arch=%_arch%';" || goto end
	
	set _temp_jdk_dir=%_temp_dir%\jdk-%_java_version%-windows-%_arch%
